...
from posts__openai import OpenAI
import numpy as np
import pandas as pd
...
def main():
    ...
    # Create an embedding column in the DataFrame based on the body of the text
    df["embedding"] = df["body"].apply(get_embedding)

    # Convert DataFrame to the desired dictionary format { [id]: embedding[] }
    embeddings_dict = df.set_index('id')['embedding'].to_dict()

    # Save the dictionary to a JSON file with the desired structure
    with open(f"data/processed/{unique_id}_embeddings.json", 'w') as json_file:
